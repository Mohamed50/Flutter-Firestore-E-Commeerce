...
class AuthViewModel extends GetxController{
  ...
  Future<void> googleSignIn() async {
   final GoogleSignInAccount? _googleUser = await _googleSignIn.signIn();
    ...
    _firebaseAuth.signInWithCredential(credential).then((value) => saveUser(value));
  }

  Future<void> facebookSignIn() async {
   final FacebookLoginResult? _facebookUser = await _facebookLogin.logIn(permissions: [FacebookPermission.email]);
    OAuthCredential credential = FacebookAuthProvider.credential(
      _facebookUser!.accessToken!.token
    );
    _firebaseAuth.signInWithCredential(credential).then((value) => saveUser(value));
  }
  ...
  void emailSignUp() {
    try{
      _firebaseAuth.createUserWithEmailAndPassword(email: email!, password: password!).then((value) => saveUser(value));
    }
    catch (e){
      print(e.toString());
      Get.snackbar("Authenticating Error:", e.toString());
